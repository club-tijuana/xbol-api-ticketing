    ...
    public class EvoPaymentService : IEvoPaymentService
    {
            ...
            CancellationToken ct = default)
        {
            ...
            var allFeeItems = request.Seats
                .SelectMany(s => validPriceItems[s.PriceListItemId].FeeList)
                .ToList();

            var subTotal = request.Seats.Sum(s => validPriceItems[s.PriceListItemId].BasePrice);
            var totalFees = allFeeItems.Where(f => f.ChargeCategory != "Tax").Sum(f => f.FeeAmount);
            var totalTaxes = allFeeItems.Where(f => f.ChargeCategory == "Tax").Sum(f => f.FeeAmount);
            var total = subTotal + totalFees + totalTaxes;
            ...
            var bookingSeats = request.Seats
                ...
                {
                    SeatKey = s.SeatKey,
                    SeatPrice = validPriceItems[s.PriceListItemId].BasePrice,
            ...
            try
            {
                ...
                order = new ModelOrder
                {
                    ...
                    Fees = [.. allFeeItems
                        ...
                        .GroupBy(f => f.FeeName)
                        .Select(g => new OrderFee { FeeType = g.Key, Amount = g.Sum(f => f.FeeAmount) })],
                    Taxes = [.. allFeeItems
                        .Where(f => f.ChargeCategory == "Tax")
                        .GroupBy(f => f.FeeName)
                        .Select(g => new OrderTax { TaxType = g.Key, Amount = g.Sum(f => f.FeeAmount) })]
                };
                ...
                if (schedule != null)
                {
                    ...
                    foreach (var seatReq in request.Seats)
                    {
                        ...
                        var pricePaid = validPriceItems[seatReq.PriceListItemId].BasePrice;
                ...
                if (schedule != null)
                {
                    foreach (var ticket in order.Tickets)
                    {
                        order.Items.Add(new ModelOrderItem
                        {
                            ...
                            PriceListItemId = bookingSeats.FirstOrDefault(s => s.SeatKey == ticket.TicketCode)?.PriceListItemId ?? 0
                        });
                    }
                }
                else if (bundle != null)
                {
                    foreach (var pass in bundlePasses)
                    {
                        order.Items.Add(new ModelOrderItem
                        {
                            ...
                            IsCourtesy = false,
                            Price = pass.Price,
                            PriceListItemId = bookingSeats.FirstOrDefault(s => s.SeatKey == pass.TrackingCode)?.PriceListItemId ?? 0
                        });
                    }
                }
            ...
            CancellationToken ct)
        {
            ...
            var clientId = contact.Id is > 0 ? contact.Id : null;

            if (clientId.HasValue)
            {
                client = await _dbContext.Clients.FindAsync([clientId.Value], ct);
                if (client is null)
                {
                    throw new KeyNotFoundException($"Client {clientId.Value} not found.");
                }
            }
